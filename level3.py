...
def level1(window, score, lives, dead):
    ...
    font2 = pygame.font.Font("Resources/Fonts/HIGHSPEED.TTF", 44)
    start_time = pygame.time.get_ticks()
    inc_diff_counter = -1 #for increasing difficulty
    enemy_fire_inc = 0
    enemy_speed_inc = 0
    enemy_reload_dec = 0
    gain_life_at = 10000
    ...
    while True:
        ...
        #rot_speed = 0.002
        backgrounds[0].blit()
        backgrounds[1].moveCircular(100, 100, 0.002*1)
        backgrounds[1].blit()
        backgrounds[2].moveCircular(100, 100, 0.002*-1)
        backgrounds[2].blit()
        """
    ...
    ############################
        ...
        if len(enemies) == 0:
            wave_counter += 1
            inc_diff_counter += 1
            if inc_diff_counter != 0 and inc_diff_counter % 2 == 0: #Every 2 waves, increase the bullets each enemy fires by 1
                if enemy_fire_inc <= 9: #Enemy cannot fire more than 10 bullets at a time
                    enemy_fire_inc += 1
            if inc_diff_counter % 4 == 0: #Every 4 waves, increase the speed of each enemy by 0.01 and decrease reload rate by 200
        ...
        highscoretext = font2.render("HIGH SCORE: " + str(highscore), True, (255, 224, 108))
        livestext = font2.render("Lives: " + str(lives[0]), True, (255, 224, 108))
        wavetext = font2.render("Level " + str(wave_counter), True, (255, 224, 108))
        window.blit(highscoretext,(window.get_width() - highscoretext.get_width(), 0))
        window.blit(scoretext,(window.get_width() - scoretext.get_width(), highscoretext.get_height()))
